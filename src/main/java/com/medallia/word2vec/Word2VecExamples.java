...
public class Word2VecExamples {
	...
	public static void main(String[] args) throws IOException, TException, UnknownWordException, InterruptedException {
		//demoWord();
		//pass the input model file path and output path
		if(args.length>=4){
			loadModel(args[1],args[3]);
		} else{
	...
	public static void loadModel() throws IOException, TException, UnknownWordException {
		//final Word2VecModel model=Word2VecModel.fromBinFile(new File(fop+"java.mod"));
		/*
		try (ProfilingTimer timer = ProfilingTimer.create(LOG, "Loading model")) {
			String json = Common.readFileToString(new File(fop+"CSvector (1).mod"));
	...
	/** Loads a model and allows user to find similar words */
	public static void loadModel(String modelFile,String outputVectorFiles) throws IOException, TException, UnknownWordException {
		//fop+"java.mod"
		final Word2VecModel model=Word2VecModel.fromBinFile(new File(modelFile));
		/*
		try (ProfilingTimer timer = ProfilingTimer.create(LOG, "Loading model")) {
			String json = Common.readFileToString(new File(fop+"CSvector (1).mod"));
			model = Word2VecModel.fromThrift(ThriftUtils.deserializeJson(new Word2VecModelThrift(), json));
		...
		*/
		interact(model.forSearch(),outputVectorFiles);
	}
	...
	private static void interact(Searcher searcher,String outputPath) throws IOException, UnknownWordException {
		
		try{
			String word = "if";			
			List<Match> matches = searcher.getMatches(word, 20,outputPath);
			System.out.println(searcher.getRawVector(word));
			System.out.println(Strings.joinObjects("\n", matches));

		}catch(Exception ex){
			ex.printStackTrace();
		}
		System.out.print("Get vector representation from model : ");		
	}
